...
static void actionup(const Arg *arg);
...
applysizehints(Client *c, int *x, int *y, int *w, int *h, int interact)
{
	...
	*h = MAX(1, *h);
	...
	if (*w < bh)
		*w = bh;
	if (resizehints || c->isfloating || !c->mon->tag[c->mon->seltag].lt[c->mon->tag[c->mon->seltag].sellt]->arrange) {
...
cleanup(void)
{
	...
	view(&a);
	selmon->tag[selmon->seltag].lt[selmon->tag[selmon->seltag].sellt] = &foo;
...
configurerequest(XEvent *e)
{
	...
	if ((c = wintoclient(ev->window))) {
		...
		else if (c->isfloating || !selmon->tag[selmon->seltag].lt[selmon->tag[selmon->seltag].sellt]->arrange) {
			m = c->mon;
...
{
	selmon->tag[selmon->seltag].nmaster = MAX(selmon->tag[selmon->seltag].nmaster + arg->i, 0);
	arrange(selmon);
}
...
movemouse(const Arg *arg)
{
	...
	do {
		...
		case MotionNotify:
			...
			if (!c->isfloating && selmon->tag[selmon->seltag].lt[selmon->tag[selmon->seltag].sellt]->arrange
			&& (abs(nx - c->x) > snap || abs(ny - c->y) > snap))
				togglefloating(NULL);
			if (!selmon->tag[selmon->seltag].lt[selmon->tag[selmon->seltag].sellt]->arrange || c->isfloating)
...
{
	int ocx1, ocy1, ocx2, ocy2, nx, ny, nw, nh, px, py, hc, vc, cc, di;
	unsigned int dui;
	Client *c;
	Monitor *m;
	Window dummy;
	...
	if (!hc && !vc)
		...
		cc = CurTopRight;
	if (XGrabPointer(dpy, root, False, MOUSEMASK, GrabModeAsync, GrabModeAsync,
		None, cursor[cc]->cursor, CurrentTime) != GrabSuccess)
		return;
	XWarpPointer(dpy, None, c->win, 0, 0, 0, 0, hc ? c->w + c->bw - 1 : -c->bw + 1, vc ? c->h + c->bw - 1 : -c->bw + 1);
	do {
		XMaskEvent(dpy, MOUSEMASK|ExposureMask|SubstructureRedirectMask, &ev);
		switch(ev.type) {
		case ConfigureRequest:
		...
		case MotionNotify:
			if ((ev.xmotion.time - lasttime) <= (1000 / 60))
				continue;
			lasttime = ev.xmotion.time;
			nx = hc ? c->x : ev.xmotion.x - 1;
			ny = vc ? c->y : ev.xmotion.y - 1;
			nw = MAX(hc ? ev.xmotion.x - ocx1 - 2 * c->bw + 1 : ocx2 - nx, 1);
			nh = MAX(vc ? ev.xmotion.y - ocy1 - 2 * c->bw + 1 : ocy2 - ny, 1);
			if (nw < c->minw) { nx = c->x; nw = c->w; }
			if (nh < c->minh) { ny = c->y; nh = c->h; }
...
restack(Monitor *m)
{
	...
	if (!m->sel)
		return;
	if (m->sel->isfloating || !m->tag[m->seltag].lt[m->tag[m->seltag].sellt]->arrange)
...
{
	if (!arg || !arg->v || arg->v != selmon->tag[selmon->seltag].lt[selmon->tag[selmon->seltag].sellt])
...
setmfact(const Arg *arg)
{
	...
	if (!arg || !selmon->tag[selmon->seltag].lt[selmon->tag[selmon->seltag].sellt]->arrange)
		return;
	f = arg->f < 1.0 ? arg->f + selmon->tag[selmon->seltag].mfact : arg->f - 1.0;
	if (f < 0.1 || f > 0.9)
		return;
	selmon->tag[selmon->seltag].mfact = f;
	arrange(selmon);
}
...
showhide(Client *c)
{
	...
	if (ISVISIBLE(c)) {
		...
		if (!c->mon->tag[c->mon->seltag].lt[c->mon->tag[c->mon->seltag].sellt]->arrange || c->isfloating)
...
{
	selmon->tag[selmon->seltag].showbar = !selmon->tag[selmon->seltag].showbar;
	updatebarpos(selmon);
...
togglefloating(const Arg *arg)
{
	...
	if (selmon->sel->isfloating) {
		if (arg)
			resize(selmon->sel, selmon->wx + arg->i, selmon->wy + arg->i,
...
{
	unsigned int i, count = 0, seltag, newtagset = selmon->tagset[selmon->seltags] ^ (arg->ui & TAGMASK);
	...
	if (newtagset) {
		...
		if (count == 1) {
			if (selmon->tag[selmon->seltag].showbar != selmon->tag[seltag].showbar) {
				selmon->seltag = seltag;
				updatebarpos(selmon);
				XMoveResizeWindow(dpy, selmon->barwin, selmon->wx, selmon->by, selmon->ww, bh);
				arrange(selmon);
			} else
...
updatesizehints(Client *c)
{
	...
	if (size.flags & PMinSize) {
		c->minw = 0;//size.min_width;
		c->minh = 0;//size.min_height;
	} else if (size.flags & PBaseSize) {
		c->minw = 0;//size.base_width;
		c->minh = 0;//size.base_height;
	} else
		c->minw = c->minh = 0;
